...
class SortedList(MutableSequence):
    ...
    def _check_order(self, idx, val):
        ...
        if idx > 0:
            ...
            if _lists[pos_prev][idx_prev] > val:
                msg = '{0} not in sort order at index {1}'.format(
                    repr(val), idx)
    ...
    def _islice(self, min_pos, min_idx, max_pos, max_idx, reverse):
        ...
        if min_pos > max_pos:
            ...
            return chain(_lists[min_pos][min_idx:],
...
class SortedListWithKey(SortedList):
    ...
    def irange(self,
        ...
        return self._irange_key(min_key=minimum,
                                max_key=maximum,
                                inclusive=inclusive,
                                reverse=reverse, )
    ...
    def extend(self, values):
        ...
        if any(keys[pos - 1] > keys[pos] for pos in range(1, len(keys))):
            raise ValueError('given sequence not in sort order')
    ...
    def _check(self):
        try:
            ...
            assert len(self._maxes) > 0 and len(self._keys) > 0 and len(
                self._lists) > 0
            ...
            assert all(
                self._key(val) == key
                for val, key in zip(
                    (_val for _val_list in self._lists
                     for _val in _val_list), (_key for _key_list in self._keys
